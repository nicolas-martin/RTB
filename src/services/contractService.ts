...
class ContractService {
  ...
  async startGame(wagerAmount: string, deadlineSeconds: number = 300): Promise<string> {
  ...
  listenToGameEvents(gameId: string, callbacks: {
    onSeedFulfilled?: (seed: string) => void;
    onRoundPlayed?: (roundIndex: number, card: number, win: boolean, newPayout: string) => void;
    ...
    if (callbacks.onRoundPlayed) {
      this.contract.on(
        ...
        (gameId, roundIndex, card, extra, win, newPayout) =>
          callbacks.onRoundPlayed!(roundIndex, card, win, ethers.formatEther(newPayout))
      );
    }
    ...
    if (callbacks.onCashedOut) {
      this.contract.on(
        ...
        (gameId, player, amount) => callbacks.onCashedOut!(ethers.formatEther(amount))
      );
    }

    if (callbacks.onBusted) {
      this.contract.on(
        this.contract.filters.Busted(gameIdBn),
        () => callbacks.onBusted!()
      );
    }
  }
